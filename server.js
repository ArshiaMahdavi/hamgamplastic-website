...
const productRoutes = require('./server/product.routes');
...
app.get('/admin/:page', (req, res) => {
  const pages = new Set(['login', 'dashboard', 'products', 'orders', 'users', 'discounts', 'settings']);
...
const registerValidationSchema = Joi.object({
  ...
  email: Joi.string().trim().lowercase().min(6).required().email(),
  password: Joi.string().min(6).max(128).required()
});

const loginValidationSchema = Joi.object({
  emailOrUsername: Joi.string().trim().min(3).max(120).required(),
  password: Joi.string().min(1).max(128).required()
});

const shippingInfoValidationSchema = Joi.object({
  firstName: Joi.string().trim().min(2).max(80).required(),
  lastName: Joi.string().trim().min(2).max(80).required(),
  phone: Joi.string().trim().pattern(/^(\+98|0)?9\d{9}$/).required().messages({
...
const createOrderValidationSchema = Joi.object({
  shippingInfo: shippingInfoValidationSchema.required(),
  products: Joi.array().items(orderProductValidationSchema).min(1).max(100).required(),
  amount: Joi.number().integer().min(1000).max(10000000000).required()
});

const orderIdValidationSchema = Joi.object({
  orderId: Joi.string().trim().max(80).required()
});

const verifyPaymentValidationSchema = Joi.object({
  orderId: Joi.string().trim().max(80).required(),
  authority: Joi.string().trim().max(80).required()
});
...
app.post('/api/register', async (req, res, next) => {
  try {
    ...
    const userExists = await User.findOne({ $or: [{ email: value.email }, { username: value.username }] });
    if (userExists) return res.status(400).send({ message: 'نام کاربری یا ایمیل قبلا ثبت شده است' });
    const hashedPassword = await bcrypt.hash(value.password, 10);
    const user = new User({ username: value.username, email: value.email, password: hashedPassword });
    await user.save();
...
app.post('/api/login', async (req, res, next) => {
    try {
        ...
        if (error) return res.status(400).json({ error: 'اطلاعات ورود معتبر نیست.' });
        const { emailOrUsername, password } = value;
        const normalizedLogin = emailOrUsername.includes('@') ? emailOrUsername.toLowerCase() : emailOrUsername;
        const user = await User.findOne({ $or: [{ email: normalizedLogin }, { username: normalizedLogin }] });
        if (!user) return res.status(401).json({ error: "نام کاربری یا رمز عبور نادرست است" });
        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) return res.status(401).json({ error: "نام کاربری یا رمز عبور نادرست است" });
        const token = jwt.sign({ _id: user._id, id: user._id, username: user.username, email: user.email, role: user.role }, process.env.JWT_SECRET, { expiresIn: '24h' });
        res.json({ message: "ورود موفقیت‌آمیز بود ✅", token, user: { id: user._id, username: user.username, email: user.email, role: user.role } });
    } catch (error) {
        next(error);
    }
});
...
app.post('/api/create-order', async (req, res, next) => {
    try {
        const { error, value } = createOrderValidationSchema.validate(req.body, { abortEarly: false });
        if (error) return res.status(400).json({ message: error.details[0].message || 'اطلاعات ارسالی برای ثبت سفارش معتبر نیست.' });
        const { shippingInfo, products, amount } = value;
        let userId = null;
        const authHeader = req.header('Authorization');
        if (authHeader) {
            try {
        ...
        const subtotal = products.reduce((sum, item) => sum + (Number(item.price) * Number(item.quantity)), 0);
        ...
        const newOrder = new Order({ user: userId, shippingInfo, products: safeProducts, subtotal, shippingCost, amount: finalAmount });
        await newOrder.save();
...
app.post('/api/request-payment', async (req, res, next) => {
    try {
        const { error, value } = orderIdValidationSchema.validate(req.body);
        if (error) return res.status(400).json({ message: 'شناسه سفارش معتبر نیست.' });
        const { orderId } = value;
        if (!ZARINPAL_MERCHANT_ID) {
            return res.status(503).json({ message: 'درگاه پرداخت در حال حاضر پیکربندی نشده است.' });
        }
        const order = await Order.findOne({ orderId: orderId });
        if (!order) return res.status(404).json({ message: 'سفارش یافت نشد.'});
        if (order.paymentStatus === 'پرداخت شده') {
            return res.status(400).json({ message: 'این سفارش قبلا پرداخت شده است.' });
        }
        ...
        try {
            ...
            callback = new URL(callback_url, `${req.protocol}://${req.headers.host}`);
        }
        callback.searchParams.set('orderId', order.orderId);
...
app.post('/api/verify-payment', async (req, res, next) => {
    try {
        const { error, value } = verifyPaymentValidationSchema.validate(req.body);
        if (error) return res.status(400).json({ success: false, message: 'اطلاعات تایید پرداخت معتبر نیست.' });
        const { authority, orderId } = value;
        if (!ZARINPAL_MERCHANT_ID) {
            return res.status(503).json({ success: false, message: 'درگاه پرداخت در حال حاضر پیکربندی نشده است.' });
        }
        ...
        if (order.paymentStatus === 'پرداخت شده') {
            return res.json({ success: true, message: 'این پرداخت قبلا تایید شده است.', order });
        }
        ...
        if ([100, 101].includes(zarinpalVerifyResp.data.data.code)) {
...
app.use((err, req, res, next) => {
    ...
    if (err && err.code === 11000) {
        return res.status(400).json({ message: 'اطلاعات وارد شده قبلا ثبت شده است.' });
    }
    res.status(500).json({
